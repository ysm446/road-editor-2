...
float computeSignedCurvatureRadiusXZ(const glm::vec3& p0, const glm::vec3& p1, const glm::vec3& p2) {
    ...
    const float signedTwiceArea = (b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x);
    const float twiceArea = std::abs(signedTwiceArea);
    if (ab < 1e-5f || bc < 1e-5f || ca < 1e-5f || twiceArea < 1e-5f)
        return 0.0f;
    const float radius = (ab * bc * ca) / (2.0f * twiceArea);
    const float sign = (signedTwiceArea >= 0.0f) ? 1.0f : -1.0f;
    return radius * sign;
}
...
float computeBankAngleRadians(float radius, float targetSpeedKmh, float friction) {
    if (std::abs(radius) <= 1e-5f)
        return 0.0f;
    const float speedMps = std::max(0.0f, targetSpeedKmh) * (1000.0f / 3600.0f);
    const float sign = (radius >= 0.0f) ? 1.0f : -1.0f;
    const float a = speedMps * speedMps / std::abs(radius);
...
std::vector<float> BankAngleGen::sampleAnglesRadians(
    ...
    for (size_t i = 0; i < curve.size(); ++i) {
        ...
        const float radius = computeSignedCurvatureRadiusXZ(p0, p1, p2);
        ...
        if (!road.bankAngle.empty()) {
            ...
            for (size_t j = 0; j < road.bankAngle.size(); ++j) {
                ...
                const float pointRadius = computeSignedCurvatureRadiusXZ(q0, q1, q2);
...
std::vector<CurveFrame> BankAngleGen::sampleFrames(
    const std::vector<CurvePt>& curve,
    const std::vector<float>& anglesRadians) {
    std::vector<CurveFrame> frames(curve.size());
    if (curve.empty())
        return frames;
    ...
    for (size_t i = 0; i < curve.size(); ++i) {
        ...
        if (curve.size() == 1) {
            ...
            tangent = curve[1].pos - curve[0].pos;
        } else if (i + 1 == curve.size()) {
            tangent = curve[i].pos - curve[i - 1].pos;
        } else {
            tangent = curve[i + 1].pos - curve[i - 1].pos;
        }

        if (glm::length(tangent) <= 1e-6f)
            tangent = (i > 0) ? frames[i - 1].tangent : fallbackForward;
        else
            tangent = glm::normalize(tangent);

        glm::vec3 baseRight = glm::cross(worldUp, tangent);
        if (glm::length(baseRight) <= 1e-6f) {
            baseRight = (i > 0) ? frames[i - 1].right : glm::vec3(1.0f, 0.0f, 0.0f);
        } else {
            baseRight = glm::normalize(baseRight);
        }
        glm::vec3 baseUp = glm::cross(tangent, baseRight);
        if (glm::length(baseUp) <= 1e-6f)
            baseUp = (i > 0) ? frames[i - 1].up : worldUp;
        else
            baseUp = glm::normalize(baseUp);

        const float angle = (i < anglesRadians.size()) ? anglesRadians[i] : 0.0f;
        const float c = std::cos(angle);
        const float s = std::sin(angle);

        CurveFrame frame;
        frame.tangent = tangent;
        frame.right = glm::normalize(baseRight * c + baseUp * s);
        frame.up = glm::normalize(baseUp * c - baseRight * s);
        frame.angleRadians = angle;
        frames[i] = frame;
    }
